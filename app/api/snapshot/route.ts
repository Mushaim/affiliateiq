...
// GET handler for Vercel Cron (cron jobs always use GET)
// Protected by checking the authorization header Vercel injects
export async function GET(req: NextRequest) {
  const authHeader = req.headers.get("authorization");
  // In production, Vercel injects CRON_SECRET as a bearer token
  ...
  try {
    ...
    return NextResponse.json({
      ...
      weekOf: new Date().toISOString().split("T")[0],
      stats: { activeAffiliates: stats.activeAffiliates, fraudFlagged: fraudFlagged.length },
      alerts,
      topAffiliates: top.map(a => ({ id: a.id, name: a.name, revenue: a.metrics.totalRevenue })),
    });
  } catch (err) {
